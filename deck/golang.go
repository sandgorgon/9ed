...
func (GoSegmenter) Segment(src []byte) []Card {
	...
	for _, d := range file.Decls {
		...
		decls = append(decls, decl{spanStart: spanStart, title: firstLine(src[declPos:]), kind: kind})
	}
	...
	for i, d := range decls {
		...
		cards = append(cards, Card{Title: d.title, Span: [2]int{d.spanStart, end}, Kind: d.kind})
	}
	return cards
}
